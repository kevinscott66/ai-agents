...
function cleanTicker(raw: string): string {
  ...
  // Аудит 2026-09-11: обрезка считала единицы UTF-16 — тот самый счёт, от
  // которого `symbolFromName` четырьмя строками выше ушла ещё 2026-08-29;
  // правку тогда не разнесли на второй путь, хотя комментарий ниже прямо
  // объявляет обе функции приведёнными к общему поведению. Тикер приходит из
  // фида (`symbol`, `gecko_id`, хвост `token` после двоеточия — все три ведут
  // сюда через `deriveSymbol`), эмодзи в нём фид приносит регулярно, и десятая
  // единица могла попасть в середину суррогатной пары: "ABCDEFGHI🚀X" давало
  // девять букв плюс одинокий верхний суррогат. В UTF-8 он не кодируется — SQLite
  // подменяет его на U+FFFD, и в БД оказывается не то, что вернула функция, а
  // читатель `/api/unlocks` видит вместо тикера битый символ.
  const out = Array.from(sym).slice(0, 10).join("").toUpperCase();
