...
describe("/digest/<id> несуществующей статьи", () => {
  ...
  test("вложенный путь под /digest/ — тоже 404, а не фолбэк", async () => {
    // Аудит 2026-09-11: тест утверждал ровно обратное («идёт обычным
    // фолбэком, поведение не менялось») и ждал 200. Написан он был, чтобы
    // очертить границу правки 2026-08-13, а не чтобы защищать этот ответ, —
    // и очертил её слишком широко: `digestIdFromPath` вложенность не матчит,
    // поэтому `/digest/a/b` уходил в `serveStatic`, а тот отдавал index.html
    // с og-тегами главной, статусом 200 и без noindex. Это та же дыра, что
    // чинил сам этот файл, просто со стороны формы пути, а не наличия статьи.
    // Разбор — в audit-2026-09-11-stray-article-path.test.ts.
