...
|---|---|---|
| `PAID_ACTION_MAX_RUB` | 1000 | Потолок одного действия для сервиса без своего потолка. Выше — отказ, владелец делает это сам. |
| `PAID_ACTION_MAX_RUB_YANDEX_GO` | 1000 | Потолок заказа такси. |
| `PAID_ACTION_MAX_RUB_YANDEX_LAVKA` | 3000 | Потолок заказа в Лавке. |
| `PAID_ACTION_MAX_RUB_YANDEX_EDA` | 3000 | Потолок заказа в Еде. |
| `PAID_ACTION_MAX_RUB_YANDEX_MARKET` | 5000 | Потолок покупки на Маркете. |
| `PAID_ACTION_MAX_RUB_YANDEX_DELIVERY` | 1000 | Потолок заказа курьера. |
...
Не выполняет действие и не знает про такси и магазины: исполнители (`agent/lib/dispatch/taxi.ts`,
`agent/lib/dispatch/shop.ts`) вызывают `claim`, `checkFinal`, `abort` и `complete`. Не хранит реквизиты карт и пароли аккаунтов.
