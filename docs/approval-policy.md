...
| --- | --- |
| `money` | `ORDER_TAXI` и `TAXI_CANCEL` (заказ дополнительно идёт через подписанный гейт); `ORDER_FOOD`, `ORDER_DELIVERY`, `MARKET_PURCHASE` (зарезервированы до шага 10); промпт `MAC_RUN_CLAUDE` про оплату/покупку/заказ |
