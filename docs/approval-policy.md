...
| --- | --- |
| `money` | `ORDER_TAXI` и `TAXI_CANCEL` (заказ дополнительно идёт через подписанный гейт); `ORDER_FOOD` (Яндекс Лавка, тоже через гейт); `ORDER_DELIVERY`, `MARKET_PURCHASE` (зарезервированы); промпт `MAC_RUN_CLAUDE` про оплату/покупку/заказ |
