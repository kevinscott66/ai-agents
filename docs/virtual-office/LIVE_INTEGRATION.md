...
Activation requires review and an explicitly approved exact commit per the local access policy. No production writes or deployment occurred while preparing this integration. Use the existing reviewed backend deployment procedure; don't copy runtime databases or credentials. Run migrations through the normal NativeAccess initialization. The two added role-binding tables contain derived routing metadata only.
