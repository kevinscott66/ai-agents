 ...
 * `.env.example:55` печатает `MEMORY_HISTORY_LIMIT=` без значения, то есть
 ...
 * `mention`. Пустой массив — «разметка есть, упоминаний в ней нет»;
 * `undefined` — «сообщения нет, судить не по чему» (тогда вызывающий вправе
 * упасть на запасной путь).
 *
