...
export function registerMessageHandler(
  ...
  const processMessage = async (ctx: Context, voice?: { text: string; native?: boolean; inputImages?: {mediaType:string;base64:string}[]; inputDocuments?: {filename:string;text:string}[]; history?: import("../lib/db.ts").ChatRow[] }) => {
    ...
    try {
      ...
      const handoffBudget: HandoffBudget = { n: 0, max: voice?.native ? Math.min(8, HANDOFF_MAX_INVOCATIONS) : HANDOFF_MAX_INVOCATIONS };
      ...
      await afterSend(def.key, async () => {
        ...
        const targets = (voice?.native ? [] : findHandoffTargets(reply, def.key, bots)).filter(
