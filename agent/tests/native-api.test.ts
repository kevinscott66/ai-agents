...
test('native API binds device to owner; rejects browser, revoked ACL and repeat execution', async () => {
  ...
  try {
    ...
    const conversationId = 'native-dialog-00000001';
    expect((await nativeApi(req('conversations', token, {id:conversationId,title:'A dialog'}), store)).status).toBe(200);
    const id = 'native-turn-00000001';
    expect((await nativeApi(req('turns', token, {id,text:'hello',conversationId}), store)).status).toBe(202);
    ...
    const index = await (await nativeApi(req('conversations', second.token), store)).json() as any;
    expect(index.conversations[0].id).toBe(conversationId);
