...
describe("блёрб дайджеста: ровно один знак конца предложения", () => {
  ...
  test("итоговый пост в канал не удваивает точку", () => {
    ...
    // Раньше за блёрбом сразу шло «Подробнее →», и удвоенная точка была видна
    // в одной строке с ссылкой. Ссылки у пункта больше нет (AUD-20260921-033),
    // но правило осталось: блёрб заканчивается ровно одним знаком.
    expect(text).toContain("\nБиржа объявила листинг.\n");
    expect(text).toContain("\nБез точки.\n");
    expect(text).not.toContain("..");
  });
});
