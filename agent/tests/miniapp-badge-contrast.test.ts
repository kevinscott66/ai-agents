 ...
 *
 *   • карточка агента в Agents.tsx — `action.status === "ok" ? "success" : …`
 *     (номер строки тут стоял и уехал: перекодировки в файле больше нет,
 *     подтверждение — проверка ниже про `badge ${action.status}`). Правила
...
describe("бейджи статусов читаемы", () => {
  ...
  test("карточка агента красит статусы тем же словарём, что Logs и Dashboard", () => {
    // Регрессия в карточке агента (Agents.tsx): "ok" → класс "success",
    // которого у бейджа нет.
