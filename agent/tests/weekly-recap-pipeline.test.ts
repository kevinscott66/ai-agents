...
describe("черновик без kind остаётся дайджестом", () => {
  ...
  test("шапка дайджеста на месте — ровно как до появления kind", () => {
    ...
    // Подводка сменилась вместе со ссылками (AUD-20260921-033): «детали по
    // ссылкам» обещали страницу выпуска, которой на живом сайте в момент
    // публикации нет. Проверяем, что ветка дайджеста — именно она, а не
    // недельная.
    expect(t).toContain("Коротко о главном — разделы сайта под постом.");
    expect(t).toContain("🔥 **Monad вышел в мейннет**");
    expect(t).toContain("Разделы: [Дайджест](https://delabs.space/digest)");
  });
