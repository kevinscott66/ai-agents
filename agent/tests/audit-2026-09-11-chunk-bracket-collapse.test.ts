 ...
 * это уже не правило.
 *
 * Цена наблюдаемая: `sendChunked` отправляет отдельным сообщением
 * «(1/4) Статус дропа», а следом настоящий текст. Квадратные скобки в
...
describe("danglingLinkStart различает усечение и разбиение", () => {
  ...
  test("подпись без закрывающей `]` отрезается и без хвоста", () => {
