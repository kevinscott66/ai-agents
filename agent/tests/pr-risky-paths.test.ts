...
describe("прежняя классификация не сломана", () => {
  ...
  test("тесты и Mini App — теперь к человеку (аудит 2026-08-11)", () => {
    ...
    // решение. `.github/workflows/auto-merge.yml` обе группы в свой белый
    // список не брал ни дня, то есть два пути автомержа отвечали по-разному на
    // один и тот же файл. Мини-апп — код, который отдаётся пользователям по
    // HTTPS; правка теста — то, чем следующий PR проходит проверку.
