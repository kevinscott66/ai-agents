 ...
 * только по `archived_at`: в cold-storage.ts `SELECT count(*)` считает,
 * сколько выгружать, `WHERE archived_at < ? AND rowid > ?` берёт страницу,
 * `DELETE … rowid <= ?` удаляет выгруженное. Остальные обращения к
...
describe("индексы холодного хранилища", () => {
  ...
  test(
    ...
    () => {
      // Удаление после успешной выгрузки — внутри `exportColdStorage`
      // (lib/cold-storage.ts). Полный скан
