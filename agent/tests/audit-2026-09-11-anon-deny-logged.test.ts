...
describe("отбой анонимного ведра виден в журнале", () => {
  ...
  beforeAll(async () => {
    ...
    // Сотни запросов подряд: на загруженном CI дефолтных 5 с хуку не хватало.
  }, 30_000);
  ...
  test("шторм не печатается построчно", () => {
    ...
    // секунду, и пока идёт слив, часть успевает вернуться. На медленном
    // раннере CI слив 360 запросов шёл ~1,5 с, и из 60 лишних отбито было 29
    // (2026-09-18, дважды подряд). Поэтому лишних 200: запас в разы больше
    // долива. Проверяем то, ради чего тест написан: отбоев много.
