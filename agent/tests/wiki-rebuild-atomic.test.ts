 ...
 * после разрушения: неверный MEMORY_DIR в /opt/agent-team/.env, непримонти-
 * рованный том, переезд каталога — и старт молча оставляет всем 12 ролям
 * пустой индекс. SEARCH_WIKI перестаёт находить что-либо, Wiki-вью Mini App
...
describe("ребилд не разрушает индекс раньше, чем убедился в источнике", () => {
  test("отсутствующий каталог памяти оставляет прежний индекс", () => {
    seedOrphanRow();
    // Ровно то, что даёт опечатка в MEMORY_DIR или непримонтированный том.
