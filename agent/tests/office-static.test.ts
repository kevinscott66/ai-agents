...
test('office static assets are gated, path-allowlisted and carry a same-origin CSP',async()=>{
 ...
 process.env.NATIVE_OFFICE_ENABLED='true';const page=await officeStatic(new URL('https://host/office/'),'GET',root);expect(await page?.text()).toBe('office');expect(page?.headers.get('content-security-policy')).toContain("connect-src 'self'");
