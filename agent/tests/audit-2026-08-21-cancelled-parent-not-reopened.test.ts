 ...
 * кнопки Mini App (task-fsm.ts:41,43), а из `running` перехода нет вовсе.
 *
 * Замер до правки:
 ...
 * закрыт там же (`dispatch/tasks.ts:116`); второй — `DELEGATE_TO_ROLE`, куда
 * `parentId` приезжает явным `_parent_task_id` и, чаще, циклом `SPLIT_TASK`:
 * тот создаёт детей по одному через gateOrDispatch, каждая итерация — полный
 * ход делегата (десятки секунд × N ролей), и владелец, нажавший «отменить» на
