 ...
 *
 * Они не совпадали. В скрипте шесть форм, в `scrubSecretString` было
 * реализовано пять: не хватало шестой, `ИМЯ=значение` — той самой, которая
...
describe("паритет со скриптами-сканерами", () => {
  for (const [name, src] of SCRIPTS) {
    test(`${name} по-прежнему знает форму присваивания`, () => {
      expect(src).toContain("(TOKEN|SECRET|PASSWORD|API_KEY|APIKEY|SESSION)");
    });
  ...
  test("все шесть форм скрипта маскируются скраббером", () => {
    const samples = [
