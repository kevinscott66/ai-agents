 ...
 *
 *   • вокабуляр статусов — закрытый набор `ACTION_STATUSES` в
 *     lib/audit.ts, из него
 *     страница сама отбрасывает `pending_approval`/`forbidden`/`rate_limited`/`approved`
...
/**
 * Ровно тот набор, что страница отбрасывает как «ещё не запуск» — читается из
 * самой страницы. Своя копия здесь уже разъезжалась бы с Mac.tsx: аудит
 * 2026-09-14 добавил туда `approved`, а копия осталась бы прежней.
 */
const NON_RUN = new Set<string>(
  [...(SRC.match(/const NON_RUN = new Set<string>\(\[([^\]]*)\]\)/)?.[1] ?? "").matchAll(/"([^"]+)"/g)].map(
    (m) => m[1],
  ),
);
...
describe("предпосылки: «выполняется» не приходит из базы", () => {
  ...
  test("переписывают только строку в полёте — терминальную не трогает никто", () => {
    ...
    // Закрытие своей строки по id, санитар брошенных по возрасту и закрытие
    // строки, которую гейт оставил ждать решения: отказом или протуханием
    // (`closeGatedActionRow`, аудит 2026-09-11) и одобрением, дошедшим до
    // диспатча (`settleApprovedActionRow`, аудит 2026-09-14).
    expect(updates).toHaveLength(4);
    // Все огорожены статусом, и статус в ограде — НЕтерминальный. Без этого
    // условия повторный вызов или санитар переписали бы уже записанный
    // результат — то есть соврали бы о том, чем действие кончилось.
    const NON_TERMINAL = ["status='attempted'", "status='pending_approval'"];
    for (const sql of updates) {
      expect(NON_TERMINAL.some((fence) => sql.includes(fence))).toBe(true);
    }
  });
});
