...
describe("предпосылки: «выполняется» не приходит из базы", () => {
  ...
  test("переписывают только строку в полёте — терминальную не трогает никто", () => {
    ...
    // Три места: закрытие своей строки по id, санитар брошенных по возрасту и
    // (аудит 2026-09-11) закрытие строки, которую гейт оставил ждать решения,
    // — отказ и протухание заявки, `closeGatedActionRow`.
