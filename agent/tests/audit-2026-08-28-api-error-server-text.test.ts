 ...
 * «строка не подействует: …» (lib/miniapp-server.ts:1689), создание задачи и
 * переход по FSM — 400 с сообщением исключения (:1322, :1355), пермишены и
 * бюджеты — 400 «bad body: …». Всё это `apiRequest` уже кладёт в `err.body`,
 * и никто не читал.
 *
 * Отдельно `POST /api/mac/stop`: 503 с телом `mac_offline` (:974) попадал под
