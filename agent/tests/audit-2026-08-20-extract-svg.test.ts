 ...
 * что кусок разбирается настоящим resvg. Вызывающий код
 * (`generateSvgFromPrompt` в lib/svg-fallback.ts) считает успехом любую непустую строку, так что мусор доезжает до рендера.
 */
