...
/**
 * Инвариант стоит в `assignTask` — это последний рубеж у самой записи. Тот же
 * отказ продублирован в хендлере в форме `{ ok: false, error }` — ровно как
 * проверка авторства у отмены рядом.
 *
 ...
 *
 * Настоящие причины дубля три: текст (модели нужно имя роли и что делать, а не
 * формулировка инварианта для читателя кода), лог (отказ по правилу оставляет
 * `[security]`-строку, исключение — только стек) и слой (правило — работа
 * диспетчера). Тесты ниже проверяют ровно это: форму ответа и то, что запись
 * не состоялась.
 */
