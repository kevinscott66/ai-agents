 ...
 * отдаёт `agent_overrides` целиком независимо от параметра `agent` (ветка
 * `GET /api/autonomy` в miniapp-server.ts, комментарий «отдаём список всегда:
 * он короткий»), а
 ...
 *
 * Цена — общее ведро GET'ов: 120 с доливом 4/с (`GET_LIMIT` в miniapp-server.ts), одно
