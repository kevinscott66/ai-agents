...
describe("cutBlock не путает скобки прозы с обрывом ссылки", () => {
  ...
  // Круг 32 отменил прежний пин «подпись без `]` тоже отрезается». Он требовал
  // ровно того, против чего заведён этот describe: непарная `[` объявлялась
  // обрывом без единого доказательства. В усечении хвоста нет вовсе, значит и
  // предъявить доказательство неоткуда — и `cutBlock` схлопывал блок до символа
  // перед `[`. Разбор — в докблоке `danglingLinkStart` (lib/telegram-format.ts).
  test("подпись без закрывающей скобки — проза, её не отрезают", () => {
    const block = "Смотри тут [очень длинная подпись ссылки без закрытия";
    const out = cutBlock(block, fitsTo(30));
    expect(out).toContain("[очень");
    expect(out.length).toBeLessThanOrEqual(31); // 30 + «…»
  });
});
