...
describe("подпись к гейту не врёт", () => {
  test("telegram-actions.ts отправляет читателя к живому пути", () => {
    ...
    //
    // Проверка подстрочная и цитату от ссылки не отличает: рассказ о старой
    // ошибке обязан обойтись без этой связки слов. Докблок telegram-actions.ts
    // так и написан, и там же сказано почему.
