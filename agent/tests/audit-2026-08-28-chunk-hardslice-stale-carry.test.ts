 ...
 * резчик выдавал бы тысячи частей по два символа. Открывашка теперь несёт
 * только метку короче семнадцати символов, прогон длиннее в неё не идёт вовсе.
 *
 * Здесь же стояло «для читателя это ничего не меняет, потому что lang-токен
 * форматтер выбрасывает» — неправда, и дорогая: выбрасывает он его только у
 * ЗАКРЫТОГО блока и только перед переводом строки, а закрывашку у непоследней
 * части дописывает сам резчик. Разбор и починка —
 * audit-2026-09-11-chunk-fence-lang-eats-content.test.ts. Этот файл пинит
 * ДЛИНЫ частей и пропажу содержимого не ловит: пустая часть под лимит
 * проходит идеально.
 */
...
describe("жёсткая резка меряет куски своей чётностью фенсов", () => {
  ...
  test("честная мерка не вырождается в тысячи частей по два символа", () => {
    // Прогон длиннее 16 символов в открывашку не попадает, поэтому место под
    // содержимое остаётся. Иначе на этом входе было бы ~2100 частей, и
    // каждая — отдельное сообщение в чат.
    const parts = splitForTelegram("хвост\n\n```" + langRun(8400), HARD, FITS);
  ...
  test("перенесённая открывашка не тащит за собой длинный lang-токен", () => {
    ...
    // Открывашка дописывается в начало каждой продолжающей части. Считай
    // резчик такой прогон языком — в неё уходил бы кусок исходного текста
    // длиной с лимит, и уходил бы столько раз, сколько частей. Считаем по
    // суммарной длине: дублирования быть не должно.
...
describe("форма исправления", () => {
  ...
  test("открывашка собирается одной функцией, и она же отсекает не-язык", () => {
