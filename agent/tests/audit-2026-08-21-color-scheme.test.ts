 ...
 *
 * Отдельный случай — `<select>` выбора режима автономии роли в
 * `pages/Agents.tsx` (тот, что зовёт `setAgentAutonomy`): единственный
 * `<select>` без какого-либо CSS-правила (`.filter-row select` его не
 * достаёт, класса нет, инлайнового фона нет). Он рисовался целиком по умолчанию движка, то есть
