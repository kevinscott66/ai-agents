 ...
 *
 * 1. Мерка искала тег ЦЕЛИКОМ — `/<svg\b[^>]*>/i`, — а `>` внутри значения
 *    атрибута законный XML (запрещён там только `<`). Сегодня на её месте
 *    `SVG_OPEN` (svg-render.ts), который ищет только открывающее `<svg`. `<svg data-x="a>b" width="100000"
