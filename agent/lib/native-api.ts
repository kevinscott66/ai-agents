...
export type NativeLead = (userId: string, text: string, reply: (text: string) => void) => Promise<void>;
...
export async function nativeApi(req: Request, injectedStore?: NativeAccess): Promise<Response> {
  ...
  if (path === '/api/native/turns' && req.method === 'POST') {
    ...
    const started = store.start(body.id, identity.device, identity.userId, body.text);
    ...
    if (started === 'created') {
      ...
      void Promise.resolve().then(() => run(identity.userId, text, answer => store.append(id, answer))).then(() => {
