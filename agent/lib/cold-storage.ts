 ...
 * шапки уходил с мыслью, что перед DELETE пересчитаны строки. Координата,
 * стоявшая здесь вместо имени, показывала на `gzipSync` и на соседний
 * комментарий «Это НЕ проверка того, что легло в файл» — то есть на код,
 * прямо опровергавший абзац, который её и привёл. Сверка живёт в
 * `exportColdStorage`, в ветке `nl !== rows.length`.
 ...
 * 12 ботов и HTTP Mini App. Экспорт запускается раз в месяц — в ПЕРВЫЙ прогон
 * месяца, какого бы числа он ни случился (маркер `cold_storage_ym`, см.
 * `claimMaintMarker` в db-maint.ts), то есть ровно тогда, когда архив накопил
 * максимум. Батч режет пик до размера страницы; конкатенация gzip-членов —
 * валидный gzip, `zcat` читает файл как одно целое.
 *
 * Аудит 2026-09-11: здесь стояло «1-го числа». Это перестало быть правдой,
 * когда расписание переехало с календаря на маркер: лежавший сутки 1 сентября
 * процесс пропускал экспорт до 1 октября, и как раз от этого ушли. Для
 * читателя разница прикладная — он ищет пик нагрузки не в тот день.
 */
 ...
 * обратной дороги в БД в репозитории нет вовсе. Тот же разбор перед
 * `startBackupScheduler` в `orchestrator/services.ts` починили 2026-08-28 —
 * здесь он остался старым, то есть бэкапы и холодное хранилище разъезжались по разным местам.
 ...
 * прогон плюс любой ручной в тот же день дают то же самое.
 *
  ...
  for (const table of ARCHIVE_TABLES) {
    ...
    try {
      ...
      for (;;) {
        ...
        // Это НЕ проверка того, что легло в файл: в `fd` ещё не записано ни
        // байта, а `back` — тот самый `gz`, что свёрнут строкой выше. Сверяется
        // ровно одно — что gzip не потерял строк при сворачивании батча;
        // проверка по батчу, а не по всему экспорту, чтобы не разворачивать
        // гигабайты обратно в память.
        //
        // Гарантию из шапки модуля («удаляем только после того, как размер на
        // диске сошёлся с числом записанных байт») даёт не она, а сверка
        // `statSync(file).size` ниже — единственная, которая смотрит на диск.
        // Снять её как дубль этой значит вернуть потерю данных при короткой
        // записи и ENOSPC.
        ...
        if (wrote !== gz.byteLength) {
          ...
          // идёт раз в месяц, когда архив максимален, и пишет в тот же
