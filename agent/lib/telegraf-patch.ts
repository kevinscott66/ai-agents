 ...
 *
 * Сайд-эффектов на импорт осталось два, и они разные: `uncaughtException` и
 * `unhandledRejection`. Импортировать этот модуль значит получить оба — второй
 * в том числе, а он глушит КАЖДЫЙ необработанный reject в процессе до
 * `log.warn`, молча и без выхода. Первый глушит шум сетевого клиента telegraf и ТОЛЬКО
...
process.on("uncaughtException", (err: any) => {
  ...
  // их чистит — `formatEntry` в lib/log.ts гонит и текст, и data через
  // `scrubSecretString`/`scrubSecrets`. Здесь чистки не было — то есть в одной функции
