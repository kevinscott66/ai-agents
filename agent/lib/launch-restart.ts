 ...
 * фикса того же дня, и читается как утверждение о сегодняшнем коде.
 *
 * Уточнение 2026-09-10: с аудита 2026-08-28 `isTelegrafNoise`
 * (telegraf-patch.ts) смотрит ТОЛЬКО стек, а список кадров включает
 * `node_modules/telegraf/` целиком — то есть `Bot is not running!`, летящий из
 * `telegraf/lib/telegraf.js`, глобальный обработчик считает шумом и глушит.
 * Оба описанных исхода — и зависание на TimeoutStopSec, и аварийный выход —
 * сегодня неверны, а инвариант от этого только жёстче: ловить обязаны ЗДЕСЬ.
 * Не пойманное тут исключение из слушателя сигнала не уронит процесс и не
 * будет замечено — останется одна warn-строка, `process.exit(0)` не
 * выполнится, остальные боты `stop()` не получат.
 */
