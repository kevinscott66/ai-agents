 ...
 * они отвечают. Сам же список pre-auth (там, где считается `preAuth` для
 * анонимного рейт-лимита) перечислял всё это верно: он говорит «не под /api/
 * ИЛИ /api/health ИЛИ OPTIONS», то есть считает по форме пути, а не по памяти.
 * Ставя маршрут ВЫШЕ стены, сверяться надо с ним, а не отсюда. С 2026-09-20
 * (AUD-030) это правило живёт в lib/miniapp-preauth.ts — вместе со списком
 * тех, кого оно не покрывает, и контрактным тестом, сверяющим обе стороны.
...
import { listTaskWindow, TASK_LIST_DEFAULT_LIMIT, TASK_LIST_MAX_LIMIT } from "./task-list-window.ts";
...
export function startMiniappServer(
  ...
  // `[\s\S]` а не `.`: приписка приходит из `first_name`, если @username не
  // задан, а имя в Telegram — произвольный текст и может содержать перенос
  // строки. С `.` такая строка шаблону не соответствовала бы и ID уехал бы
  // наружу целиком — то есть через поле, которое человек задаёт себе сам.
  // Цифры id закреплены отдельной группой, всё после первого пробела —
 ...
 */
  ...
  async function route(
    ...
    //
    // Mini App этим эндпоинтом ПОЛЬЗУЕТСЯ: `Settings.tsx` зовёт
    // `api.budgetSettings()` в общем `Promise.all` загрузки страницы.
    //
    // Аудит 2026-09-11: здесь стояло обратное — «объявлен, но не вызывается
    // ни из одного компонента, так что гейт ничего не ломает». На этом
    // основании 403 считали никому не видимым. Он видим: у каждого
    // допущенного не-админа страница настроек показывала красную ошибку с
    // кнопкой «Повторить», которая не могла сработать никогда. Гейт остаётся
    // (причина выше — `updated_by`), а 403 читается клиентом как штатный
    // ответ зрителю: `overridesFailure` в Settings.tsx.
    ...
    if (path === "/api/tasks" && method === "GET") {
      ...
      const limit = parseIntOr(url.searchParams.get("limit"), TASK_LIST_DEFAULT_LIMIT, TASK_LIST_MAX_LIMIT);
      ...
      // Выборка окна, `truncated` и `nextOffset` — lib/task-list-window.ts
      // (AUD-030); здесь остаются разбор запроса и redaction.
      const w = listTaskWindow({ assignee: assigneeKey, chatId, statuses, limit, offset });
      return json({
        tasks: redactContent(user, w.tasks, TASK_CONTENT_FIELDS),
  ...
  const server = Bun.serve({
    ...
    async fetch(req, server) {
      ...
      try {
        ...
        // Само правило живёт в lib/miniapp-preauth.ts: оно считает по ФОРМЕ
        // пути, и там же перечислены маршруты выше стены, которые правило не
        // покрывает, потому что ограничивают себя сами. Здесь его нарочно нет
        // заново — расходиться нечему (AUD-030).
        const preAuth = isPreAuthRequest(url.pathname, req.method);
        if (preAuth) {
          const limited = anonLimit(req, peer);
