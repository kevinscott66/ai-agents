 ...
 * HMAC-ом по lib/miniapp-auth.ts. Исключений ТРИ, и все до этой стены:
 ...
 * Здесь было сказано сперва «все», потом «исключение ровно одно», потом «два»
 * — и каждый раз аудит по такому описанию проходил мимо очередного входа.
 * Первый же пересчёт (2026-09-11) нашёл пропущенный OPTIONS, стоящий на
 * четыре строки РАНЬШЕ остальных; сам же список pre-auth в этом файле (там,
 * где считается preAuth для логирования) его перечислял. Пересчитывать надо
 * каждый раз, когда маршрут ставят ВЫШЕ стены.
 *
