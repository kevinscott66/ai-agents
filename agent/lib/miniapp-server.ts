 ...
 * они отвечают. Правило же, по которому запрос попадает в анонимное ведро,
 * перечисляло всё это верно: «не под /api/ ИЛИ /api/health ИЛИ OPTIONS», то
 * есть по форме пути, а не по памяти. Ставя маршрут ВЫШЕ стены, сверяться надо
 * с ним, а не отсюда; с 2026-09-20 (AUD-030) оно вынесено в
 * lib/miniapp-preauth.ts вместе со списком тех, кого правило не покрывает,
 * и сверяется с кодом контрактным тестом.
