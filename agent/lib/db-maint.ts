 ...
 *
 * Аудит 2026-09-11: здесь стояло «`deleted > inserted` — это норма: строку
 * могли скопировать в прошлый прогон, упавший между INSERT и DELETE». Такого
 * прогона быть не может. Оба стейтмента идут в ОДНОЙ транзакции и под одним
 * предикатом `where`; оборвись процесс между ними — SQLite откатит INSERT, и
 * строка останется только в источнике. Ключи тоже не переиспользуются:
 * `messages.id` — INTEGER PRIMARY KEY AUTOINCREMENT (счётчик монотонен даже
 * после удаления), у остальных источников id TEXT-овый UUID. То есть
 * `deleted` и `inserted` в нынешней форме всегда совпадают, и объяснение
 * прикрывало расхождение, которого не бывает.
 *
 * Расходиться может другая пара — `inserted` и `selected`: `INSERT OR IGNORE`
 * молчит не только на конфликте ключа, но на ЛЮБОМ нарушении ограничений,
 * включая NOT NULL и CHECK в архивной таблице. Пропущенная так строка не
 * попадает и под DELETE (его `EXISTS` не выполнится), то есть остаётся в
 * источнике. Ровно это и означает предупреждение ниже: тревожно, когда после
 * прогона строки старше cutoff остались на месте, — и сравнивается там
 * `deleted` с `selected`, а не с `inserted`.
 *
 ...
 * Но общей отсечки мало: у agent_actions нет `extraWhere`, а здесь он есть, и
 * решённую пару «заявка + её действие» он не задевает, а нерешённую — рвёт.
 * Заявка, дождавшаяся неработающего санитара (случай абзацем выше), остаётся
 * в живой таблице, пока её действие уезжает в архив, и её `action_id` с этого
 * момента указывает в `agent_actions_archive`. Аудит 2026-09-11: здесь было
 * сказано, что пара уезжает «вместе, а не половинками» — это отменяло границу
 * `status <> 'pending'`, объявленную двумя абзацами выше, ровно там, где она и
 * работает. Чтение половинку переживает: `APPROVAL_SELECT` в approvals.ts
 * джойнит обе таблицы через COALESCE — см. его докблок, там разобрано зачем.
 */
...
export function archiveOldRows(opts: ArchiveOpts = {}): ArchiveResult {
  ...
  // Шаг на шаг, а не цепочка вызовов без изоляции. `runDaily` ставит суточный
  // маркер ДО работы, поэтому упавший вызов уносил с собой все следующие до
  // конца суток: agent_actions ловил блокировку — остальные таблицы не
  // архивировались вовсе. Соседние шаги в этом файле (`_gcTick`, шаги
  // `runDaily`) давно разведены именно так.
  //
  // Числом шаги здесь НЕ считаем. Абзац писали на трёх вызовах, четвёртый
  // (`role_runtime_queue`) добавили ниже и прозу не тронули — и следующий,
  // кто заведёт пятую таблицу, по этому тексту не понял бы, что счёт уже
  // разъехался. Ровно та же причина, по которой из соседних докстрок убрали
  // номера строк.
  //
  ...
  if (failures.length > 0) {
    // Бросаем ПОСЛЕ всех шагов: до этой строки уцелевшие таблицы уже
...
export function dbStats(): TableStat[] {
  ...
  try {
    ...
    // — при пустом `MEMORY_DB_PATH=` (так он и стоит в `.env.example`)
    // `statSync("")` бросал,
 ...
 *
 * Аудит 2026-08-20: `_schedulerLastRun` пишется только изнутри
 * `startMaintScheduler` — на старте (T-411) и в конце каждого gc-тика; тестовый
 * сеттер `setSchedulerLastRunForTests` не в счёт. Именно второе и делает метку
 * меткой живости, а не однократной отметкой о запуске.
 * При `DB_MAINT_ENABLED=false` планировщик не стартует
...
export function startMaintScheduler(
  ...
  const runDaily = (today: string, force = false): boolean => {
    ...
    // Обычный тик claim'ит окно атомарно; ручной force запускается даже при
    // уже существующем маркере.
    //
    // Круг 32: force возвращал `false` при неудачной записи маркера, то есть
    // отменял ВЕСЬ суточный прогон — архив, gcMessages, холодное хранилище,
    // VACUUM — ровно в той ситуации, ради которой ручной запуск и зовут. Это
    // последний экземпляр поведения, которое докстринг `claimMaintMarker`
    // описывает как убранное: «любой отказ записи выключал весь суточный
    // прогон целиком». Ветка `claim === "error"` ниже делает обратное и
    // объясняет почему — здесь тот же довод.
    if (force) {
      writeMaintMarker(DAILY_MARKER_KEY, today);
    } else {
  ...
  return {
    ...
    _runDailyNow() {
      ...
      //
      // Круг 32: кэш ставился ДО прогона и безусловно, хотя инвариант
      // тика — «кэш ставится по РЕЗУЛЬТАТУ» — записан тридцатью строками выше.
      // Прогон, отменённый на первом же шаге, всё равно закрывал сутки в
      // памяти процесса: до полуночи по UTC тик уходил в ранний return.
      //
      // Круг 42: при `force` runDaily возвращает true всегда — маркер там
      // пишется и не проверяется, а ветки `taken`/`error` в форс-путь не
      // заходят вовсе. То есть `else` ниже сегодня не срабатывает, и знать об
      // этом важнее, чем вычистить его: он повторяет условие тика тридцатью
      // строками выше и ждёт того дня, когда форс научится отказываться —
      // например, когда его повесят на админ-команду с собственной проверкой
      // прав. Тогда кэш не должен закрывать сутки по отменённому прогону.
      const today = ymd(nowProvider());
      if (runDaily(today, true)) lastDailyYmd = today;
      else lastDailyYmd = null;
    },
    _gcTick,
    _alertingHourlyTick,
