 ...
 * `system_prompt` хранит целиком системный промпт временной роли (дубль того,
 * что уже лежит в `tasks.input`), и остаётся в живой БД бессрочно после того,
 * как роль отработала.
 *
 ...
 *
 * Событие ОДНО на весь проход, а не по строке на заявку: первый проход после
 * включения TTL закрывает всю накопленную очередь разом, а оба подписчика
 * (Approvals, Dashboard) на каждое событие делают полный перезапрос списка.
 * Сотня событий превратилась бы в сотню запросов с телефона за то же самое.
 * Ни один подписчик payload не читает — им нужен факт «список изменился», —
 * поэтому в payload лежат все id разом.
 */
