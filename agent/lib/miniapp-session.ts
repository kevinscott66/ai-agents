...
export class MiniAppSessionStore {
   ...
   * не поймать.
   *
   * Аудит 2026-08-28: cookie выдаётся в хвосте пост-обработки
   * `miniapp-server.ts` — уже ПОСЛЕ `catch` самого обработчика. Ответ там
   * готов и корректен: тело посчитано, gzip и ETag наложены, CORS проставлен.
   * Бросок в этом месте уходил прямо в `Bun.serve`, у которого в этом сервере
   * нет опции `error`: наружу вместо готового ответа уходил голый 500, а
   * строка access-лога (она печатается в том же хвосте) не писалась вовсе,
   * так что в логах оставалась дырка вместо запроса.
   *
   * Аудит 2026-09-11: тем же фиксом хвост обёрнут собственным `try/catch`
   * (`[miniapp] post-processing error`), так что до `Bun.serve` бросок больше
   * не доходит, — но выдавать 500 на готовом ответе он всё равно будет.
   * Прямых вызовов `cookie()` из сервера не осталось ни одного, и это
   * состояние закреплено тестом (audit-2026-08-28-miniapp-response-tail).
   *
