...
export const TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    name: "MAC_RUN_CLAUDE",
    description:
      "Запустить Claude Code или Codex CLI на личном Mac оператора. provider выбирает первого исполнителя. allowFallback=true (по умолчанию для нового запроса) разрешает резервный Codex, только если Claude недоступен ДО выполнения: нет авторизации/бинарника либо проверка готовности подтвердила лимит или проблему оплаты. При явном запрете менять исполнителя передай allowFallback=false. После timeout, обрыва, отказа прав или уже начавшегося исполнения не повторяй задачу через другого агента. Автоматический переход Codex→Claude запрещён: его sandbox строже. Для Codex ask/plan = read-only, accept_edits/auto = workspace-write, без интерактивных запросов разрешений; bypass не поддерживается. Только orchestrator может вызывать; пользователь должен быть в whitelist MAC_USER_IDS. Начинай с plan, если не уверен: это единственный режим, который ничего не исполняет.",
    input_schema: {
      type: "object",
      properties: {
        project: {
        ...
        provider: {type:"string",enum:["claude","codex"],description:"Первый исполнитель на Mac. По умолчанию claude; для Codex обязательно codex."},
        allowFallback: {type:"boolean",description:"Разрешить резервного Codex, если Claude не может начать выполнение. По умолчанию true для новых запросов. false при явном запрете менять исполнителя. Не расширяет права и не повторяет начатые действия."},
