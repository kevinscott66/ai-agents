 ...
 * Запускается асинхронно после ответа агента НЕ КОРОЧЕ 30 символов
 ...
 * значит молча потерять запись агента.
 *
 * Дверей две, и обещание «голову режем, но не выбрасываем, и ставим отметку»
 * держится только в первой — когда под голову вообще остаётся место. Во
 * второй (`room <= 0`: одна свежая секция уже заполняет лимит) возврат не
 * содержит НИ головы, НИ отметки: на странице остаётся обрезанная свежая
 * запись и больше ничего. Это осознанный размен — пустая страница хуже
 * обрезанной записи, а места под отметку тем более нет, — но выглядит он
 * как нетронутая короткая страница, и читателю, пришедшему сюда искать
 * потерю данных вики, надо знать про эту дверь сразу.
 */
 ...
 *
 * Аудит 2026-09-11: оба обещания даны здесь без оговорок, а исключение из них
 * описано этажом ниже — в `keepNewestOnly`, ветка `room <= 0`: там не остаётся
 * ни головы, ни отметки. Первым читают этот докблок, поэтому дверь названа и
 * тут. По штатному пути компактора она недостижима (`clip(content,
 * MAX_PAGE_CONTENT)` держит секцию около 830 символов при лимите 12_000), но
 * страница с гигантской «секцией» — например, если `WRITE_WIKI` записал текст
 * с подстрокой-разделителем внутри — в неё попадает.
 */
export function trimMergedPage(page: string, limit = MAX_MERGED_PAGE): string {
  ...
  // дальше — их и убираем.
  //
  // «Сохраняем» — про факт, а не про эту конкретную строку. Там, где в ЭТОМ
  // прогоне что-то выброшено, `staleNote` не возвращается: свежая отметка
  // говорит ровно то же самое («со страницы унесли секции»), и две подряд —
  // это и есть тот мусор, от которого чистим. Возвращается она там, где иначе
  // не сказал бы никто: `dropped === 0` и ветка без секций ниже.
  ...
  if (!sections.length) {
    // Сюда попадают только из-за самих отметок: секций нет, голова в лимит
    // влезает, а страница — нет, то есть лишними были ровно накопившиеся
    // отметки. Старое `head.slice(0, limit)` возвращало голову, вычищенную от
    // них ВСЕХ, — и страница, с которой уже уносили секции, становилась
    // неотличима от нетронутой. Одну возвращаем на место.
    return cutAt(head, limit - staleNote.length).trimEnd() + staleNote;
  }
