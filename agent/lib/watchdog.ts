 ...
 *
 * Каждый бот должен периодически получать Telegram-апдейты (как минимум
 * чужие сообщения в чате). Если за `silenceMs` от какого-то бота не было
 * ни одного [raw] апдейта — алертим в чат через `alert`. Чтобы не спамить,
 * после алёрта суппрессим этот key на 1 час.
 */
...
export function startWatchdog(deps: WatchdogDeps): { stop: () => void } {
  ...
  // возврата промиса `.catch` ниже не поймает — он вылетит из колбэка целиком.
  // Молчащий бот не повод ронять сервис.
