...
export const nativeTurnContext = new AsyncLocalStorage<{ userId: string; turnId:string; conversationId:string; knowledge?:string; readKnowledge?:(query:string)=>string; writeKnowledge?:(write:{scope:'conversation'|'project';id:string;kind:'fact'|'decision'|'task';text:string})=>unknown; reply?:(agentKey:string,text:string)=>Promise<{message_id:number;date:number}>; linkApproval: (id:string) => void; mediaSink?: NativeArtifactSink }>();
