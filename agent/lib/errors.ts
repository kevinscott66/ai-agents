 ...
 *  Часть вызовов оборачивает Bot API (PUBLISH_TO_CHANNEL и соседи), а
 ...
 *  отдаётся админам через /api/actions. Чистим здесь, а не по вызывающим:
 *  точка ровно одна, и будущие вызовы закрыты по построению.
 *
 *  Аудит 2026-09-11, круг 51: обе фразы выше называли число — «часть из 75
 *  вызовов» и «не в 75 местах». На день правки вызовов 125 (120 вне tests/).
 *  Числа убраны по правилу круга 20: счёт, записанный прозой, устаревает
 *  молча, а смысл абзаца от него не зависит — важно, что вызывающих много и
 *  что чинить их поимённо нельзя. Замер держит
 *  tests/audit-2026-09-11-lying-comments-tier1.test.ts.
 *
