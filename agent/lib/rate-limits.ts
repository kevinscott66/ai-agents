 ...
 *
 * Каждый бакет — массив записей `BucketEntry` (см. ниже), а НЕ голых чисел:
 * кроме времени запись несёт `reservationId`, пока слот удерживает резервация
 * ведра юзербота. При проверке отбрасываем всё старше windowMs и считаем
 * количество. Если >= max → reject.
 *
...
const RULES: Record<string, { perAgent?: BucketRule; global?: BucketRule }> = {
  ...
  DELIVERY_STATUS: { perAgent: { windowMs: MINUTE_MS, max: 6 } },
};
 ...
 *
 * Аудит 2026-09-11: вызывающие — ветки «агентский бакет отказал после того,
 * как чат-слот занят». Там стоял `refundChatRateLimits`, то есть для
 * GENERATE_IMAGE — no-op, при комментарии, обещающем возврат. Первой нашли
 * ветку в gateOrDispatch; аудит 2026-09-14 — её близнеца в ретрае self-diag
 * (`processDiagTask`), где она к тому же достижима. Других поводов звать эту
 * функцию нет: у «резервация не использована» узкий и проверяемый смысл, а
 * «верни слот за неудачу» — это соседняя функция, и сливать их обратно нельзя.
 */
