...
/**
 * Отказ на уровне соединения: ответа мы не увидели.
 *
 ...
 *
 * Аудит 2026-09-11 (круг 29): здесь стояло «запрос не доехал — ретраить
 * безопасно и нужно», и первая половина этой фразы неверна. Мы знаем только,
 * что не доехал ОТВЕТ. Обрыв на чтении тела выглядит точно так же — это тот
 * самый случай, ради которого заведён пер-запросный `AbortSignal.timeout`
 * (клиентский `timeout` считает одни заголовки, см. докблок таймаута выше), и
 * приходит он сюда как `APIConnectionError` либо как undici `terminated`.
 * Такой запрос сервер принял, выполнил и посчитал.
 *
 * Значит ретрай здесь — до `TRANSIENT_MAX_RETRIES` — может оплатить один и тот
 * же вызов дважды, а при неудаче трижды. Ключа идемпотентности нет: SDK 0.98
 * его не принимает, и ставить заголовок вслепую хуже, чем назвать цену. Цена
 * осознанная: ветка заведена, чтобы дрогнувший сокет не ронял ход целиком, а
 * сторона, которая дублируется, — деньги, не действия. Побочные эффекты у нас
 * исполняет вызывающий по tool_use, и второго исполнения из повтора запроса
 * не выходит. Убирать ретрай ради биллинга — решение владельца, не рефакторинг.
 */
    ...
    while (true) {
      try {
        ...
        if (isTransientFailure(e)) {
          ...
          // единственная из трёх — спала, ДЕРЖА слот. Соседи — ветки `429` и
          // `5xx` выше по этому же циклу — отпускают его с 2026-08-13 (круг 29:
          // тут стояли номера строк 312 и 328, к тому времени 550 и 570;
          // координата без имени файла не видна ни одному из двух сторожей,
          // потому и тухла молча). Инвариант записан в
