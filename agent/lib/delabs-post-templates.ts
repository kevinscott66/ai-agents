 ...
 * она в тулах как `${SITE_BASE}/digest/${id}` — см. `SITE_BASE` в
 * tools/weekly-draft.ts. База без схемы давала `[Подробнее
 * →](delabs.space/digest/1)`, пустая — `(/digest/1)`. Для Telegram это href
 * без схемы: ответ «unsupported URL protocol», то есть не отправляется ВЕСЬ
 * пост, а не теряется одна ссылка.
 *
 * Через `DELABS_SITE_BASE` этот пример больше не воспроизвести: `delabsSiteBase`
 * (delabs-env.ts) с 2026-08-29 сам проверяет схему и на всём, что ей не
 * отвечает, берёт адрес по умолчанию. Проверка здесь остаётся второй линией —
 * `SITE_BASE` не единственный источник адреса в пункте, а этот докблок
 * обещает отсеивать не-ссылку, кто бы её ни принёс. Скобка внутри адреса
 ...
 * `parseDigestPost` в lib/site-ingest.ts ссылается на вывод как на пример. Докблок в
