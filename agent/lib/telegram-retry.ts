 ...
 *
 * `launchWithRestart` (launch-restart.ts) в поле не нуждается, но не потому,
 * что ноль там безвреден сам по себе. Там `retry_after` поднимает собственную
 * задержку и тут же ограничивается сверху:
 * `Math.min(Math.max(waitMs, retryAfterMs), MAX_RETRY_AFTER_WAIT_MS)`. При
 * дефолтном `maxDelayMs` (60 с — ниже пятиминутного потолка) ноль не меняет
 * ничего: `Math.max` возвращает уже посчитанный `waitMs`, `Math.min` не
 * кусается. При `maxDelayMs` выше потолка ноль заводит ветку, и кламп срежет
 * набранную экспоненциальную паузу до пяти минут. Пол `MIN_RETRY_AFTER_SECONDS`
 * туда не доезжает: `parseRetryAfterSeconds` отдаёт сырое `secs >= 0`, а пол
 * применяется только внутри ретраера этого файла.
 */
