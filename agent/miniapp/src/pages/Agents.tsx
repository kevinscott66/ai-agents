 ...
 * параметра `agent` (miniapp-server.ts:1712, «отдаём список всегда»), а из
 ...
 *
 * Цена: ведро GET'ов рейт-лимита — 120 с доливом 4/с (miniapp-server.ts:554),
...
export default function Agents() {
  ...
  async function loadAgentAutonomy(keys: string[]) {
    ...
    try {
      ...
      // (miniapp-server.ts:1712) — параметр `agent` на состав ответа не
      // влияет, а эффективный режим страница не читает вовсе. Общий бакет GET
      // — 120 с доливом 4/сек (miniapp-server.ts:554), и админ, щёлкающий
