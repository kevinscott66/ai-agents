...
а заказ такого тарифа — отказ. Комментарий, контакты, кнопка заказа и статусы — «НЕ сверено»:
...
в шапке, стоимость доставки и карточки меню (`product-card-v2-*`); счётчик и
«минус» на карточке, корзина, оформление, оплата и заказы — предположения,
