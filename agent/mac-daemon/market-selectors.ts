 ...
 *
 * Счётчик на карточке, строки корзины и страница оформления сверены живьём
 * (сентябрь 2026): товар клали в корзину, доходили до оформления и убирали
 * обратно. Кнопку оплаты не нажимали ни разу.
 *
 * НЕ сверено: статусы заказа, включая приезд в пункт выдачи и обещанное время,
 * — их видно только после оплаты. Сверяет владелец
 * (`bun mac-daemon/shop.ts probe market`), правится только этот файл. Не нашёл
 * элемент — отказ до оплаты со скриншотом.
 */
...
export const MARKET_TESTID = {
  ...
  // Оформление. Слова «Итого» на странице нет вовсе: итог лежит в
  // `summaryTotalPrice`, а рядом подпись способа — «Оплата онлайн 142 ₽».
  checkoutTotal: '[data-auto="summaryTotalPrice"]',
  payButton: '[data-auto="payButton"]',
  // Способы оплаты: панель со списком, выбранный отмечен `input:checked`.
  paymentPanel: '[data-auto="payment-options-panel"]',
  paymentMethod: '[data-zone-name="paymentMethod"]',
};
...
export const MARKET_TEXT = {
  ...
  /** Сверено: кнопка в корзине подписана «Перейти к оформлению». */
  checkout: /^(?:Перейти к оформлению|Оформить(?: заказ)?|К оформлению)/,
  /** Запасной путь к кнопке оплаты, если `payButton` пропадёт из вёрстки. */
  pay: /^(?:Подтвердить заказ|Оплатить|Оформить и оплатить|Пополнить и оплатить)/,
  savedCard: /(?:•{2,}|\*{2,}|··)\s?\d{4}|Сбер ?Пэй|SberPay|Яндекс ?Пэй|Yandex ?Pay/i,
  payOnDelivery: /При получении/i,
  /**
   * Денег на выбранном способе не хватает: кнопка тогда подписана «Пополнить и
   * оплатить», и заплатить без владельца нельзя.
   */
  topUpNeeded: /Пополнить|Не хватает/i,
...
export const MARKET_STATE_TEXT: ReadonlyArray<[ShopOrderState, RegExp]> = [
  ...
  // «Заказ доставлен в пункт выдачи» — это ещё не выдача, потому и оговорка.
  ["delivered", /Заказ (?:доставлен|получен)(?! в пункт)|Получен|Вручён/i],
  // Посылка доехала до ПВЗ и ждёт владельца — отдельная стадия, и проверяется
  // она раньше «в пути»: «Заказ в пункте выдачи» встречается вместе с историей
  // «был в пути», и общее правило перетянуло бы его на себя.
  ["pickup_ready", /(?:Ждёт|Ожидает) (?:вас )?в пункте выдачи|Можно забирать|Готов к выдаче|Прибыл в пункт выдачи|Заказ в пункте выдачи|Доставлен в пункт выдачи/i],
  ["delivering", /Передан в доставку|В пути|Курьер (?:в пути|едет)/i],
