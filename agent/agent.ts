...
async function run() {
  ...
  if (args.mode === "review") {
    ...
    // Fail-closed. Блок `AUTO_CONTROL_LOOP` в
    // deploy/vps-autonomous/autonomous-cycle.sh (тот, что помечен T-513)
    // обещает: «если GitHub недостижим — не начинать новую работу», и держится
    // это на `set -e` вокруг вызова. Номера строк здесь стояли и протухли
    // (270-275 при настоящих 306-312) — правило круга 20: не подгонять номер,
    // а назвать место так, чтобы оно пережило вставку выше. Но бросает только первый `gh pr list`; ошибки отдельных PR
