...
// Stop closes the socket as well as the loops; a blocked ReadMessage must wake.
...
func (t *YandexDocsTransport) scheduleReconnect(attempt int) {
	...
	t.RecordReconnect()
	// Bound retries for invalid/expired documents and transient carrier failures.
	delay := time.Second * time.Duration(1<<min(attempt, 5))
	time.AfterFunc(delay, func() { t.connectToDoc(attempt + 1) })
}
...
func (t *YandexDocsTransport) fetchDocInfo(url, userID string) (YandexDocsInfo, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return YandexDocsInfo{}, fmt.Errorf("document HTTP status %d", resp.StatusCode)
	}
	const maxDocumentBytes = 8 << 20
	htmlBytes, err := io.ReadAll(io.LimitReader(resp.Body, maxDocumentBytes+1))
	if err != nil {
		return YandexDocsInfo{}, fmt.Errorf("cannot read document")
	}
	if len(htmlBytes) > maxDocumentBytes {
		return YandexDocsInfo{}, fmt.Errorf("document too large")
	}
	...
	return parseDocInfo(htmlBytes, strings.Join(cookies, "; "), userID)
}

func parseDocInfo(htmlBytes []byte, cookies, userID string) (YandexDocsInfo, error) {
	...
	if err := json.Unmarshal([]byte(matches[1]), &config); err != nil {
		return YandexDocsInfo{}, fmt.Errorf("invalid document configuration")
	}
	balancerURL := config.OfficeAction.BalancerURL
	balancer, err := url.Parse(balancerURL)
	if err != nil || balancer.Scheme != "https" || balancer.Hostname() == "" || balancer.User != nil || balancer.Port() != "" || (balancer.Path != "" && balancer.Path != "/") || balancer.RawQuery != "" || balancer.Fragment != "" {
		return YandexDocsInfo{}, fmt.Errorf("invalid document balancer")
	}
	host := balancer.Host
	document := config.OfficeAction.Editor.Document
	key, ok := document["key"].(string)
	token := config.OfficeAction.Editor.Token
	if !ok || key == "" || token == "" {
		return YandexDocsInfo{}, fmt.Errorf("incomplete document configuration")
	}
	...
	return YandexDocsInfo{
		...
		Origin:      balancerURL,
		Host:        host,
		WsURL:       fmt.Sprintf("wss://%s/2024.1.1-375/doc/%s/c/?EIO=4&transport=websocket", host, url.PathEscape(key)),
		Permissions: perms,
		OpenCmd: map[string]interface{}{
			"c":      "open",
			"id":     key,
