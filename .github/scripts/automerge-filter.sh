...
#    Аудит 2026-09-11: полторы недели эта проверка существовала только здесь,
#    то есть нигде — живой путь (validatePrChecklist в
#    agent/lib/dispatch/github.ts) личности не смотрел вовсе. Теперь смотрит,
#    из той же переменной и с тем же значением по умолчанию; согласие двух
#    редакций держит agent/tests/audit-2026-09-11-automerge-author-gate.test.ts.
#
...
is_safe_path() {
  case "$1" in
    # docs/* без оговорки про расширение пускало бы шелл-скрипт из docs/:
    # каталог документации — это документация по содержимому файлов, а не по
    # имени каталога (аудит 2026-09-11, там же сужен isAutoMergeable()).
    docs/*.md|README*.md) return 0 ;;
