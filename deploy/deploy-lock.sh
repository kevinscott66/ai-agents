...
#
# Зачем. Выкатывать в /opt/agent-team умеет один путь — `deploy/deploy.sh` с
# Mac владельца. Запусков у него бывает несколько сразу: две сессии, два
# воркри, повтор после Ctrl-C; ни о каком общем состоянии они не знают.
#
# Второй путь был: воркфлоу deploy.yml в CI со своим rsync и своим рестартом.
# Воркфлоу сериализовались между собой группой `concurrency: group:
# deploy-vps`, а Mac про эту группу не знал вовсе — ради этого зазора замок и
# заводили. Аудит 2026-09-11: воркфлоу удалён при публичном релизе 2026-09-01,
# в прод из CI не ходит больше ничто, и замок остался нужен уже по первой
# причине, а не по этой. Раньше абзац здесь описывал выкатку из CI в настоящем
# времени — читатель мог ждать, что push в main доедет до прода сам.
#
...
#   DEPLOY_LOCK_SSH        команда ssh с опциями (default "ssh"); выкатка из CI
#                          передавала сюда ssh с явным ключом и
