...
# Та же запись, что в deploy.sh, и по той же причине (AUD-027): /opt/web3-puls
# не git-чекаут, и без манифеста «какой коммит сейчас на сайте» знал только
# терминал оператора. Общий скрипт, а не копия команды: две копии этого места
# разъехались бы молча. Провал записи выкатку не валит — код уже уехал.
if [ -x "$SCRIPT_DIR/version-manifest.sh" ]; then
  if ! DEPLOY_RECORD_PATH="$REMOTE" DEPLOY_RECORD_REPO="$REPO_ROOT" \
       "$SCRIPT_DIR/version-manifest.sh" record site; then
    red "манифест версии сайта не записался — выкатку это не отменяет"
  fi
else
  red "нет $SCRIPT_DIR/version-manifest.sh — версия выкатки сайта нигде не останется"
fi
