...
# --- запас диска на проде (до смоука: отказ должен быть быстрым) ---
# Аудит AUD-20260919-026. Корневой раздел прода — 9.6 ГБ, и выкатка на нём не
# самый крупный, но самый опасный едок: ENOSPC посреди шага 3 оставляет
# node_modules в состоянии, из которого сервис не поднимается, а откат по
# снапшоту сам требует места. Поэтому спрашиваем не «сколько свободно», а
# «останется ли резерв, если выкатка займёт своё».
#
# Откуда 400 МиБ. Замерено на проде 2026-09-20: снапшот кода ~17 МиБ, полезная
# нагрузка rsync ~20 МиБ, сборка Mini App ~50 МиБ вместе с временными файлами
# vite, остальное — запас на `bun install`, который при смене зависимостей
# переписывает часть из 546 МиБ node_modules и наполняет кэш. Цифра с запасом
# намеренно: ошибиться в меньшую сторону здесь дороже, чем отложить выкатку.
#
# Проверка идёт ДО смоука: смоук — это пара минут тестов, и узнавать про
# нехватку места после них незачем.
DISK_NEED_MIB="${DEPLOY_DISK_NEED_MIB:-400}"
if [ ! -x "$SCRIPT_DIR/disk-guard.sh" ]; then
  red "❌ нет $SCRIPT_DIR/disk-guard.sh — без проверки запаса диска выкатка не начнётся."
  red "   Похоже на частичный чекаут: проверьте, что репозиторий цел."
  exit 1
fi
cyan "== запас диска на $HOST (нужно $DISK_NEED_MIB МиБ) =="
DISK_RC=0
"$SCRIPT_DIR/disk-guard.sh" --host "$HOST" ${DEPLOY_SSH_KEY:+--key "$DEPLOY_SSH_KEY"} \
  --need "$DISK_NEED_MIB" --label deploy || DISK_RC=$?
# rc=1 — предупреждение: места хватает, но запас тает. Это не повод не катить;
# повод — посмотреть кандидатов в deploy/disk-budget.md и решить, что убрать. rc=2 — места
# нет, и выкатка останавливается здесь, до замка и до первого rsync.
if [ "$DISK_RC" -ge 2 ]; then
  red "❌ на проде не хватает места под выкатку — в прод не поехало ничего."
  red "   Сторож ничего не удаляет: кандидаты на уборку — deploy/disk-budget.md."
  red "   Осознанно снизить требование: DEPLOY_DISK_NEED_MIB=<МиБ> deploy/deploy.sh"
  exit 1
fi

# --- смоук перед выкаткой (локально, до замка) ---
# Аудит 2026-09-11. Между чекаутом и `systemctl restart` на проде стоял ровно
# один гейт — шаг «Pre-deploy smoke (local)» в воркфлоу deploy.yml. Воркфлоу
# удалён при публичном релизе 2026-09-01 вместе со всей выкаткой из CI, а сам
# .github/scripts/pre-deploy-smoke.sh остался: файл на месте, тесты на него
# есть, вызывать его перестало быть некому. То есть подстраховка выглядела
# существующей, а прогонов не делала ни одного: после релиза выкатка идёт
# только отсюда, и отсюда она катила и перезапускала юнит, не прогнав ни
# одного теста.
#
# Поэтому смоук зовём здесь и ДО замка: тесты идут пару минут, держать всё это
# время прод-замок незачем — вторая выкатка ждала бы на пустом месте.
if [ ! -x "$REPO_ROOT/.github/scripts/pre-deploy-smoke.sh" ]; then
  red "❌ нет $REPO_ROOT/.github/scripts/pre-deploy-smoke.sh — без смоука выкатка не начнётся."
  red "   Похоже на частичный чекаут: проверьте, что репозиторий цел."
  exit 1
fi
cyan "== смоук: bun install + bun test + сборка Mini App (локально) =="
# Обхода нет намеренно, как и у замка ниже: аварийный путь — не эта выкатка, а
# откат по снапшоту (подсказку печатает шаг 3).
if ! "$REPO_ROOT/.github/scripts/pre-deploy-smoke.sh" "$REPO_ROOT"; then
  red "❌ смоук не прошёл — в прод не поехало ничего."
  exit 1
fi

# --- 0. взаимный замок против второй выкатки ---
# Катить в /opt/agent-team умеет один путь — этот скрипт с Mac владельца.
# Запусков у него бывает несколько: две сессии, два воркри, повтор после
# Ctrl-C. Замок против них и стоит. Второй путь, выкатка из CI воркфлоу
# deploy.yml, удалён 2026-09-01 вместе с группой `concurrency: deploy-vps`,
# которая сериализовала выкатки между собой; ни один живой воркфлоу в прод не
# ходит (agent/tests/audit-2026-09-11-predeploy-smoke-unwired.test.ts).
#
